...
import {
  CreateDefaultWorkingHourInput,
  UpdateDefaultWorkingHourInput,
} from "../types/default-working-hour";
...
class DefaultWorkingHourService extends TransactionBaseService {
  ...
  async list(
    ...
    const defaultWorkingHourRepo = this.manager_.getCustomRepository(
      this.defaultWorkingHourRepository_
    );
  ...
  async retrieve(defaultWorkingHourId, config) {
    ...
    const defaultWorkingHourRepo = manager.getCustomRepository(
      this.defaultWorkingHourRepository_
    );

    const defaultWorkingHour = await defaultWorkingHourRepo.findOne(
      defaultWorkingHourId,
      config
    );
  ...
  async create(
    defaultWorkingHourObject: CreateDefaultWorkingHourInput
  ): Promise<DefaultWorkingHour> {
    return await this.atomicPhase_(async (manager) => {
      ...
      const { ...rest } = defaultWorkingHourObject;

      console.log(">> Create", rest);

      try {
        let defaultWorkingHour: any = defaultWorkingHourRepo.create(rest);
        defaultWorkingHour = await defaultWorkingHourRepo.save(
          defaultWorkingHour
        );
  ...
  async getDefaultWorkingHourSlotTimesByLocationId(locationId: string) {
    ...
    for (const dwh of dwhList) {
      ...
      if (isWorkingDay) {
        const defaultWorkingHour = divideTimes(
          selectSlotTimeFrom,
          selectSlotTimeTo,
          5
        );
