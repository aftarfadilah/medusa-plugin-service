...
class DivisionService extends TransactionBaseService {
  ...
  async list(
    ...
    const divisionRepo = this.manager_.getCustomRepository(
      this.divisionRepository_
    );
  ...
  async retrieve(divisionId: string, config: FindConfig<Division>) {
    ...
    console.log("Find division with", divisionId, config);
  ...
  async create(divisionObject: CreateDivisionInput): Promise<Division> {
    ...
    if (isExist.length > 0)
      throw new MedusaError(
        "CONFLICT",
        "Division Already Exists for this location_id and calendar_id combination"
      );

    return await this.atomicPhase_(async (manager) => {
      const divisionRepo = manager.getCustomRepository(
        this.divisionRepository_
      );

      const { ...rest } = divisionObject;
